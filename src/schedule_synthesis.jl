...
utilization = sum(c -> c.minhits/c.windowsize, constraints)
	...
	a = _scheduler_automaton(controllers..., cores=cores)
	...
	for step in 1:steps
		...
		if isempty(next_states)
			# some_non_readable_nonsense = [map(l -> _state_separation(l, [c.windowsize for c in constraints], indigits=true), reverse(path)) |> collect for path in values(current_states)]
...
"""
Build a controller automaton for the given weakly hard constraint
"""
function _controller_automaton(c::RealTimeScheduling.WeaklyHardConstraint)
	...
	_Automaton(L, 2, T, L-1, !iszero)
end

"""
Build a scheduler automaton from a given list of controller automata
"""
function _scheduler_automaton(controllers::Vector{_Automaton}; slotsize::Int64=1)
